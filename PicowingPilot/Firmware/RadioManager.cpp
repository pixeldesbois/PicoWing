...
void RadioManager::setRecvCallback(void (*cb)(const uint8_t*, const uint8_t*, int)){
    // Exemple simple : on peut stocker un pointeur statique pour utiliser dans la callback statique
    instance->recvCallback = cb;
}
...
void RadioManager::onDataRecvStatic(const uint8_t* mac, const uint8_t* data, int len){
    if(instance && instance->recvCallback) instance->recvCallback(mac,data,len);
}
